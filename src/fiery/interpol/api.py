...
from .jit_utils import jitscript, meshgrid, movedim1
...
def grid_pull(
    input: Tensor,
    grid: Tensor,
    interpolation: Union[OrderLike, Sequence[OrderLike]] = 'linear',
...
def spline_coeff(
    input: Tensor,
    interpolation: Union[OrderLike, Sequence[OrderLike]] = 'linear',
    bound: Union[BoundLike, Sequence[BoundLike]] = 'dct2',
    dim: int = -1,
    inplace: bool = False,
) -> Tensor:
...
def affine_grid(mat: Tensor, shape: Sequence[int]) -> Tensor:
    ...
    if mat.shape[-2] not in (nb_dim, nb_dim + 1):
        raise ValueError(
            'First argument should be matrices of shape '
