...
# internals
from .jit_utils import jitscript, movedim1
...
def dct2_initial(inp, pole: float, dim: int = -1, keepdim: bool = False):
    ...
    #
    # I (YB) unwarped and simplified the terms so that I could use a dot
    ...
    poles = torch.as_tensor(pole, dtype=inp.dtype, device=inp.device)
    poles = poles.pow(torch.arange(0, n, dtype=inp.dtype, device=inp.device))
